...
public class SdCardStorageAdapter extends AbstractStorageAdapter
{
  ...
  private static final String BASE_EXTERNAL_OLD = "/storage/extSdCard/grazerss";
  ...
  private void setupMedium()
  {
    ...
    mounted = Environment.MEDIA_MOUNTED.equals(status) || Environment.MEDIA_MOUNTED_READ_ONLY.equals(status) ? true : false;
    ...
    Log.d(TAG, String.format("setupMedium status=%s mounted %s->%s, read-only %s->%s, shared %s -> %s.", status, oldMounted, mounted,
        oldReadOnly, readOnly, oldShared, shared));
  ...
  public static boolean isAdvisable()
  {
    ...
    boolean mounted = Environment.MEDIA_MOUNTED.equals(status) || Environment.MEDIA_MOUNTED_READ_ONLY.equals(status) ? true : false;
